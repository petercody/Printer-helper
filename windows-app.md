# Printer Connect — Windows tray app
...
```
dist\Printer Connect Setup 1.0.0.exe
```
...
> app lands in `dist\win-unpacked\Printer Connect.exe`.
...
```
%APPDATA%\Printer Connect\config.env
```
...
- Closing the window only hides it; the agent keeps running in the tray. Use
  **Quit Printer Connect** to stop it fully.
